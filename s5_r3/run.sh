#!/usr/bin/env bash
# uni: ez2313
# adapted from provided  pretrained script
...
if [ $stage -le 6 ]; then
  ...
  for set in test dev train; do
    dir=data/$set
    steps/make_lpc.sh --nj 30 --cmd "$train_cmd" $dir
...
if [ $stage -le 19 ]; then
  echo "start run stage 19"
  scripts/make_encoder_decoder.sh
  echo "run stage 19 DONE"
fi

#train encoder-decoder
if [ $stage -le 20 ]; then
  echo "start run stage 20"
  scripts/train_encoder_decoder.sh
  echo "run stage 20 DONE"
fi


#test encoder-decoder using hires LPC test data
if [ $stage -le 21 ]; then
  echo "start run stage 21"
  scripts/test_encoder_decoder.sh
  echo "run stage 21 DONE"
fi

#prepare frontend training input and output
if [ $stage -le 22 ]; then
  echo "start run stage 22"
  scripts/prepare_frontend.sh
  echo "run stage 22 DONE"
fi

#train frontend
if [ $stage -le 23 ]; then
  echo "start run stage 23"
  scripts/train_frontend.sh
  echo "run stage 23 DONE"
fi

#test frontend
#if [ $stage -le 24 ]; then
 # echo "start run stage 24"
 # scripts/test_frontend.sh
 # echo "run stage 24 DONE"
#fi



#TODO
#test and reconstruct audio
#if [ $stage -le 21 ]; then
#  echo "start run stage 21"
#  scripts/test_encoder_decoder.sh
#  echo "run stage 21 DONE"
...
#if [ $stage -le 22 ]; then
 # echo "start run stage 22"
 # scripts/make_decoder.sh
 # echo "run stage 22 DONE"
#fi

#text to speech embedding nnet

#pass into decoder/combine nnets

#input xvectors (voxceleb) into decoder
#DONE!!!
