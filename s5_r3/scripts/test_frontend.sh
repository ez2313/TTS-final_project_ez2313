...
if [ $stage -le 2 ]; then
  echo "Begin stage 2: Compute actual encoder output"
  ...
  echo "End stage 2: Computed actual encoder output"
fi
...
if [ $stage -le 3 ]; then
  ...
  scripts/compute_MSE.py ${home}/${eval_dir}/predicted_output.ark ${home}/${eval_dir}/actual_output.ark $feature_order >>results.txt ||exit 1 
