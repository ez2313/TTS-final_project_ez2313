// Uni: ez2313 modified lightly from feature-plp.cc
// feat/feature-lpc.cc
...
LpcComputer::LpcComputer(const LpcOptions &opts):
...
LpcComputer::LpcComputer(const LpcComputer &other):
...
const MelBanks *LpcComputer::GetMelBanks(BaseFloat vtln_warp) {
...
const Vector<BaseFloat> *LpcComputer::GetEqualLoudness(BaseFloat vtln_warp) {
...
void LpcComputer::Compute(BaseFloat signal_raw_log_energy,
                          BaseFloat vtln_warp,
  ...
  residual_log_energy = std::max<BaseFloat>(residual_log_energy,
                                 std::numeric_limits<float>::min());
  raw_cepstrum_ = lpc_coeffs_;
  feature->Range(1, opts_.lpc_order).CopyFromVec(
      raw_cepstrum_.Range(0, opts_.lpc_order));
  (*feature)(0) = residual_log_energy;

//feature->Range(1, opts_.lpc_order).CopyFromVec(lpc_coeffs_.Range(0, opts_.lpc_order));
//  feature->Range(0, opts_.lpc_order).CopyFromVec(lpc_coeffs_);
 // (*feature)(0) = residual_log_energy;
  
   /* if (opts_.htk_compat) {  // reorder the features.
    BaseFloat log_energy = (*feature)(0);
    for (int32 i = 0; i < opts_.lpc_order; i++)
      (*feature)(i) = (*feature)(i+1);
    (*feature)(opts_.lpc_order)  = log_energy;
  } */
