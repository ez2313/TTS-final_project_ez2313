// Uni:ez2313 lightly modified from  feat/feature-plp.h, default lpc order changed to 20
...
class LpcComputer {
 public:
  typedef LpcOptions Options;
  explicit LpcComputer(const LpcOptions &opts);
  LpcComputer(const LpcComputer &other);

  const FrameExtractionOptions &GetFrameOptions() const {
    return opts_.frame_opts;
  }

  int32 Dim() const { return opts_.lpc_order; }
 ...
 private:
  ...
  // Disallow assignment.
  LpcComputer &operator =(const LpcComputer &other);
};

typedef OfflineFeatureTpl<LpcComputer> Lpc;
