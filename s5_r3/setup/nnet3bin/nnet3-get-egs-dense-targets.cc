...
static bool ProcessFile(const GeneralMatrix &feats,
  ...
  for (size_t c = 0; c < chunks.size(); c++) {
    ...
    KALDI_WARN << "Start_frame_subsampled is: " << start_frame_subsampled 
               << " num_frames_subsampled is: "
               << num_frames_subsampled << "targets length" << targets.NumRows();

    KALDI_ASSERT(start_frame_subsampled + num_frames_subsampled - 3 <
                 targets.NumRows());
